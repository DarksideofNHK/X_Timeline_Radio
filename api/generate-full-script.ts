...
【重要：固有名詞の読み方】
音声合成（TTS）で正しく読み上げられるよう、固有名詞は以下のルールで出力してください：

1. **人名**：
   - 読みにくい漢字の人名は「漢字、読み方、」の形式で出力
   - 例：「岸田文雄、きしだふみお、総理」→「岸田文雄、きしだふみお、総理」
   - 一般的な名前（田中、山田など）はそのままでOK

2. **外国人名・外来語**：
   - 必ずカタカナで表記
   - 例：Elon Musk → イーロン・マスク、Trump → トランプ

3. **企業・組織名**：
   - 英字略称は読み方を併記：「NASA、ナサ」「WHO、ダブリューエイチオー」
   - アルファベット企業名はカタカナ化：OpenAI → オープンエーアイ、Google → グーグル

4. **地名**：
   - 難読地名は読み方を追加：「神戸、こうべ」「大分、おおいた」

5. **数字・記号**：
   - 数字は日本語で：「100万」→「ひゃくまん」または「100万」のまま
   - 「@ユーザー名」は「アットマーク〇〇さん」と読む

※ファクトチェック：投稿内の固有名詞（人名、企業名、事件名など）が実在するか確認し、正しい読み方で出力してください。
