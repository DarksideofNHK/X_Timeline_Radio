 ...
 *
 * 使い方:
 *   npx ts-node scripts/generate-show.ts
 *
 * 必要な環境変数（.envファイルに設定）:
 *   GROK_API_KEY - X/Grok API キー
...
// 出力ディレクトリ
const OUTPUT_DIR = path.join(__dirname, '../public/shows');

// ジャンル設定
const GENRES = ['trending', 'politics', 'economy', 'lifestyle', 'entertainment', 'science', 'international'];
const GENRE_INFO: Record<string, { name: string; icon: string; prompt: string }> = {
  trending: { name: '今バズってる話題', icon: '🔥', prompt: '今バズってる話題、トレンド、面白いネタ' },
  politics: { name: '政治ニュース', icon: '🏛️', prompt: '日本の政治ニュース、政策、選挙' },
  economy: { name: '経済・マネー', icon: '💹', prompt: '経済ニュース、株価、為替、投資' },
  lifestyle: { name: '暮らし・生活', icon: '🏠', prompt: '暮らし、生活、健康、グルメ、旅行' },
  entertainment: { name: 'エンタメ', icon: '🎬', prompt: 'エンタメ、芸能、音楽、映画、アニメ、ゲーム、VTuber' },
  science: { name: '科学・テクノロジー', icon: '🔬', prompt: '科学、テクノロジー、AI、宇宙、環境' },
  international: { name: '国際ニュース', icon: '🌍', prompt: '国際ニュース、海外の話題、世界情勢' },
};

// Grok APIで投稿を収集（Vercel APIと同じ実装）
async function collectPosts(genre: string): Promise<any[]> {
  const info = GENRE_INFO[genre];
  console.log(`[Collect] ${info.icon} ${info.name}...`);
...
`;
  ...
  try {
    ...
    // output配列からテキストを抽出（Vercel APIと同じ方法）
    let fullText = '';
    ...
    if (jsonText) {
      ...
      console.log(`[Collect] ${info.icon} ${postsArray.length}件取得`);
    ...
    console.log(`[Collect] ${info.icon} JSONが見つかりませんでした`);
    return [];
  } catch (e) {
    console.error(`[Collect] Error for ${genre}:`, e);
    return [];
  }
}

// Gemini APIでスクリプト生成（Vercel APIと同じプロンプト）
...
async function generateAudio(text: string, outputPath: string, voice: string = 'nova'): Promise<void> {
  ...
  if (!response.ok && response.status === 400) {
    console.log(`[TTS] gpt-4o-mini-tts failed, falling back to tts-1`);
...
async function main() {
  ...
  console.log('📻 X Timeline Radio - 番組生成');
  console.log('='.repeat(50));

  // 環境変数チェック
  if (!GROK_API_KEY) {
    console.error('❌ GROK_API_KEY または XAI_API_KEY が設定されていません');
    process.exit(1);
  }
  if (!GEMINI_API_KEY) {
  ...
  if (!OPENAI_API_KEY) {
    console.warn('⚠️ OPENAI_API_KEY が設定されていません（音声生成をスキップ）');
  }

  // 日付ベースの出力ディレクトリ
  const now = new Date();
  const dateStr = now.toISOString().split('T')[0];
  const showDir = path.join(OUTPUT_DIR, dateStr);
  ...
  // 台本を保存
  const scriptPath = path.join(showDir, 'script.json');
  fs.writeFileSync(scriptPath, JSON.stringify(script, null, 2));
  console.log(`📝 台本保存: ${scriptPath}`);
  ...
  if (OPENAI_API_KEY) {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      try {
        ...
        console.error(`[TTS] エラー (${section.id}):`, e);
      }
    }
  } else {
    console.log('\n⏭️ 音声生成をスキップ（OPENAI_API_KEY未設定）');
  }
  ...
  const manifestPath = path.join(showDir, 'manifest.json');
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));

  console.log('\n' + '='.repeat(50));
  console.log('✅ 番組生成完了！');
  console.log(`📁 出力先: ${showDir}`);
  console.log('='.repeat(50));
}
